...
### P0-3 兩套 Harness 並行 —— 部分採納（接縫先行），全量倒置暫緩

審計對依賴方向的觀察屬實（hermes_tcm → hermes_shanghan 26 處，
反向 0 處）。本輪落地的是**接縫**：DomainPack 新增
`evidence_normalizer` / `entity_linker` 可執行接縫，內核經統一分發
調用（不再新增散落 import）。完整的依賴倒置（DomainPack Interface +
shanghan 作為標準插件 + 舊 HarnessRunner 降級為兼容適配器）是一次
大遷移，涉及測試釘死的 legacy 兼容面（36 工具、CLI/Web 語義），
不宜和 P0 修復混在一個變更集——如實標記為後續工程項，方向按審計
建議的架構圖執行。
...
### P1-1 語義/圖擴展檢索空實現 —— 維持現狀（有意的誠實邊界）

`not_implemented` + `status: planned` 正是本倉庫的反冒充原則：
向量召回沒有配套蘊含核驗與評測之前，以確定性檢索降級混充語義檢索
才是缺陷。審計建議的檢索棧（歸一 → BM25 → dense → 圖擴召 → rerank →
逐字核驗）方向正確，作為 roadmap 保留；「向量命中只能作召回信號、
不能直接作證據」與本倉庫證據不變量一致。
...
### P1-3 MCP 仍主要暴露舊系統 —— 暫緩（如實記錄）

統一 V2 MCP Server（建立在 TCMClient/RunController/RunStore 上、
durable tasks）是獨立工程項；現有 stdio server 的「非 durable tasks」
已在代碼中如實聲明。與 P0-3 的 Harness 統一同批次做，避免兩次遷移。
...
* 新增回歸：`tests/test_audit_fixes.py`（29 項，逐條對應上述修復）；
* 全量：728 項測試通過（含 legacy 兼容面 36 工具、id 格式、
  文檔計數守衛）；
